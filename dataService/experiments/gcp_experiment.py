...
def handle_client(client_socket, logger: logging.Logger):
...
def test_tuple_throughput(logger: logging.Logger):
    # Create a TCP socket
    ...
    handle_client(client_socket, logger)
...
def test_gcp(image_name: str, logger: logging.Logger):
    boot_time_test = threading.Thread(target=test_boot_time_gcp, args=(image_name, logger))
    tuple_throughput_test = threading.Thread(target=test_tuple_throughput, args=(logger,))
